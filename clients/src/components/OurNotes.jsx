...
const Notes = () => {
  ...
  return (
    <div className='notes bg-bgColor dark:bg-bgColor2 h-[100vh] max-w-full text-textPrimary dark:text-textPrimary2 flex items-center justify-center'>
      <div className="notes-child h-[60%] w-[80%] flex flex-col items-center">
        ...
        <div ref={gsapRef} className="notes-card h-[50%] w-full mt-[3%] flex items-center justify-between ">
          {
            notesCard.map((key) => {
              return <Link to={key.href} className='h-full w-[32%] bg-secondry text-textPrimary hover:scale-105 hover:shadow-lg hover:shadow-accent '>
                <div className="card h-full w-full pl-[2%] border-[3px] border-solid border-accent rounded-lg">
                  <div className="icon text-[3rem] text-accent">{key.icon}</div>
                  <h1 className='my-[1%] text-primary font-bold text-xl'>{key.head}</h1>
